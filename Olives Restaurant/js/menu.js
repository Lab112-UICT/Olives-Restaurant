/* ═══════════════════════════════════════════════
   ...
   All dish data lives here — no separate HTML
   files per dish. The detail panel is filled
   dynamically from this data object.
   ═══════════════════════════════════════════════ */


/* ──────────────────────────────────────────────
   1. DISH DATA
   Add or edit dishes here. Each dish needs:
   id, cat, name, tag, rating, price,
   shortDesc, fullDesc, ingredients[],
   nutrition{}, image URL
   ────────────────────────────────────────────── */
const DISHES = [
  /* ── Appetizers, Wings & Bites ── */
  {
    id: 1, cat: "Appetizers",
    name: "Shrimp Tempura",
...
function buildCard(dish) {
  ...
  /* Open detail panel on card click (not the + button) */
  card.addEventListener("click", function (e) {
    if (e.target.closest(".menu-card__plus")) return; // + button handles its own click
    openDetail(dish.id);
  });
...
/* Render all sections into #menuGrid */
function renderMenu(dishes) {
  const grid = document.getElementById("menuGrid");
  grid.innerHTML = "";

  // Group dishes by category, preserving CAT_META order
  const groups = {};
  ALL_CATS.forEach(function (cat) { groups[cat] = []; });
  ...
  ALL_CATS.forEach(function (cat) {
    ...
    items.forEach(function (dish) {
      sectionGrid.appendChild(buildCard(dish));
    });
...
function updateCount(n, isAll) {
  ...
  if (isAll) {
    el.textContent = "Showing all " + n + " dishes";
  } else {
    el.textContent = n + " " + (n === 1 ? "dish" : "dishes") + " found";
  }
}
...
/* Category buttons */
document.querySelectorAll(".cat-nav__btn").forEach(function (btn) {
  btn.addEventListener("click", function () {
    document.querySelectorAll(".cat-nav__btn").forEach(function (b) { b.classList.remove("active"); });
...
function openDetail(id) {
  ...
  /* Fill content */
  document.getElementById("dCat").textContent    = dish.cat;
  document.getElementById("dName").textContent   = dish.name;
  document.getElementById("dRating").textContent = "★ " + dish.rating.toFixed(1) + " rating";
  document.getElementById("dTag").textContent    = dish.tag;
  document.getElementById("dDesc").textContent   = dish.fullDesc;
  document.getElementById("dPrice").textContent  = fmt(dish.price);
  document.getElementById("dPhoto").src          = dish.image;
  document.getElementById("dPhoto").alt          = dish.name;
  document.getElementById("qtyVal").textContent  = 1;

  /* Ingredient pills */
  const ingWrap = document.getElementById("dIngredients");
  ingWrap.innerHTML = dish.ingredients.map(function (i) {
    return '<span class="detail-panel__ingredient-pill">' + i + '</span>';
  }).join("");

  /* Nutrition */
  const nut = dish.nutrition;
  document.getElementById("dNutrition").innerHTML = `
    <div class="detail-panel__nut-item">
      <span class="detail-panel__nut-val">${nut.cal}</span>
      <span class="detail-panel__nut-label">Calories</span>
    </div>
    <div class="detail-panel__nut-item">
      <span class="detail-panel__nut-val">${nut.protein}</span>
      <span class="detail-panel__nut-label">Protein</span>
    </div>
    <div class="detail-panel__nut-item">
      <span class="detail-panel__nut-val">${nut.carbs}</span>
      <span class="detail-panel__nut-label">Carbs</span>
    </div>
    <div class="detail-panel__nut-item">
      <span class="detail-panel__nut-val">${nut.fat}</span>
      <span class="detail-panel__nut-label">Fat</span>
    </div>
  `;

  /* Populate mobile back bar dish name */
  document.getElementById("mobileBarName").textContent = dish.name;
...
document.getElementById("qtyPlus").addEventListener("click", function () {
  if (qty < 20) { qty++; document.getElementById("qtyVal").textContent = qty; }
});

/* Add to Order from detail panel */
...
/* ──────────────────────────────────────────────
   7. CART — localStorage-backed array
   ────────────────────────────────────────────── */
const CART_KEY = "olives_cart";

/* Load cart from storage on page init */
function cartLoad() {
  try { return JSON.parse(localStorage.getItem(CART_KEY)) || []; }
  catch (e) { return []; }
}

/* Save current cart array to storage */
function cartSave(items) {
  localStorage.setItem(CART_KEY, JSON.stringify(items));
}

/* Add qty of a dish — increments if already in cart */
function cartAddItem(dish, qty) {
  var items = cartLoad();
  var existing = items.find(function (i) { return i.id === dish.id; });
  if (existing) {
    existing.qty += qty;
  } else {
    items.push({
...
/* Total item count across all entries */
function cartCount() {
  return cartLoad().reduce(function (sum, i) { return sum + i.qty; }, 0);
}

function updateCartBadges() {
  var total = cartCount();
  var cartCountEl  = document.getElementById("cartCount");
  var mobileBadge  = document.getElementById("mobileBadge");
  ...
  if (total > 0) {
    ...
    mobileBadge.removeAttribute("hidden");
  } else {
    cartCountEl.setAttribute("hidden", "");
    mobileBadge.setAttribute("hidden", "");
  }
}

/* Wire nav cart button to cart page */
...
renderMenu(DISHES);
attachRevealObserver();
updateCartBadges(); /* reflect any existing cart from localStorage */