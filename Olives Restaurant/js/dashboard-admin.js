...
(function () {
    ...
    var adminSidebarToggle = document.getElementById('sidebarToggle');

    function closeSidebar() {
        if (sidebar) sidebar.classList.remove('open');
        if (backdrop) backdrop.classList.remove('open');
        if (adminSidebarToggle) adminSidebarToggle.classList.remove('open');
    }
    ...
    function renderKanban() {
        ...
        LIVE_ORDERS.forEach(function (o) { (cols[o.status] || cols.delivered).push(o); });
        ...
        document.querySelectorAll('.kanban-card__advance').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                var order = LIVE_ORDERS.find(function (o) { return o.id === id; });
                ...
                if (badge) badge.textContent = LIVE_ORDERS.filter(function (o) { return o.status === 'new'; }).length || '';
            });
        });
    }
    ...
    function openDishModal(dish) {
        ...
        if (titleEl) titleEl.textContent = dish ? 'Edit Dish' : 'Add New Dish';

        var fields = {
            'dmName': dish ? dish.name : '',
            'dmCat': dish ? dish.cat : 'Italian',
            'dmPrice': dish ? dish.price : '',
            'dmTag': dish ? dish.tag : '',
            'dmRating': dish ? dish.rating : '',
            'dmDiscount': dish ? dish.discount : 0,
            'dmShortDesc': dish ? dish.shortDesc : '',
            'dmFullDesc': dish ? dish.fullDesc : '',
            'dmIngredients': dish ? (dish.ingredients || []).join(', ') : '',
            'dmImage': dish ? dish.image : ''
        };
        ...
        var availEl = document.getElementById('dmAvailable');
        if (availEl) availEl.checked = dish ? dish.available : true;

        var modal = document.getElementById('dishModal');
        var back = document.getElementById('dishModalBackdrop');
    ...
    if (addDishBtn) addDishBtn.addEventListener('click', function () { openDishModal(null); });

    ['dishModalClose', 'dishModalCancel', 'dishModalBackdrop'].forEach(function (id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('click', closeDishModal);
    });

    var saveDishBtn = document.getElementById('dishModalSave');
    if (saveDishBtn) {
        saveDishBtn.addEventListener('click', function () {
            ...
            var updated = {
                ...
                name: name,
                cat: (document.getElementById('dmCat') || {}).value || 'Italian',
    ...
    if (addPromoBtn) {
        addPromoBtn.addEventListener('click', function () {
            ...
            var back = document.getElementById('promoModalBackdrop');
            if (modal) modal.hidden = false;
            if (back) back.hidden = false;
        });
    }
    ...
    ['promoModalClose', 'promoModalCancel', 'promoModalBackdrop'].forEach(function (id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('click', closePromoModal);
    });

    var savePromoBtn = document.getElementById('promoModalSave');
    if (savePromoBtn) {
        savePromoBtn.addEventListener('click', function () {
            ...
            promoData.push({
                ...
                scope: (document.getElementById('pmScope') || {}).value || 'All',
    ...
    function renderCustomers(search) {
        ...
        var rows = q ? CUSTOMERS.filter(function (c) { return c.name.toLowerCase().includes(q) || c.email.toLowerCase().includes(q); }) : CUSTOMERS;
